...
def main_menu_keyboard():
    # Kalau admin sudah setup WEBAPP_URL (lihat config.py), tombol "Lihat Paket
    # VIP" membuka Mini App (halaman HTML tabel asli, sesuai contoh yang
    # diminta) alih-alih tabel teks di chat. Kalau belum diisi, FALLBACK
    # otomatis ke perilaku lama (callback_data="show_vip") supaya bot tetap
    # jalan normal walau WebApp-nya belum di-setup.
    if config.WEBAPP_URL:
        vip_button = InlineKeyboardButton("💎 Lihat Paket VIP", web_app=WebAppInfo(url=config.WEBAPP_URL))
    else:
        vip_button = InlineKeyboardButton("💎 Lihat Paket VIP", callback_data="show_vip")
