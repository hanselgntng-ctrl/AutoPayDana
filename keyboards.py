...
def vip_list_keyboard():
    ...
    package_buttons = [
        InlineKeyboardButton(f"🟢 {pkg['name']}", callback_data=f"buy_{pkg['id']}")
        for pkg in packages
    ]
...
def package_pick_keyboard(prefix: str):
    ...
    # dipakai warna merah supaya konsisten sebagai aksi destruktif, prefix
    # lain (mis. "editpkg") dipakai warna biru sebagai aksi netral/info.
    color = "🔴" if prefix == "delpkg" else "🔵"
    packages = db.list_packages()
    buttons = [
        [InlineKeyboardButton(f"{color} {p['name']} (Rp{p['price']:,})".replace(",", "."), callback_data=f"{prefix}_{p['id']}")]
        for p in packages
    ]
    return InlineKeyboardMarkup(buttons)
