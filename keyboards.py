...
def format_vip_table(bot=None) -> str:
    ...
    Kolom hanya: No | Nama | Harga -- TIDAK ADA kolom/pengecekan status admin
    channel lagi. Bot SENGAJA tidak memanggil Bot API apa pun (mis.
    get_chat_member) ke channel/group tujuan paket (`target_chat_id`) hanya
    untuk menampilkan tabel ini -- satu-satunya interaksi bot ke channel VIP
    tetap murni saat MEMBUAT invite link 1 kali pakai di send_package_link()
    (bot.py), sesuai kebutuhan.
    ...
    def border(left: str, joint: str, right: str) -> str:
        segs = ["─" * (W_NO + 2), "─" * (W_NAMA + 2), "─" * (W_HARGA + 2)]
    ...
    header = f"│ {cell('No', W_NO)} │ {cell('Nama', W_NAMA)} │ {cell('Harga', W_HARGA)} │"

    lines = [top_border, header, mid_border]
    for idx, pkg in enumerate(packages, start=1):
        harga = f"Rp{pkg['price']:,}".replace(",", ".")
        lines.append(
            f"│ {cell(idx, W_NO)} │ {cell(pkg['name'], W_NAMA)} │ {cell(harga, W_HARGA)} │"
        )
...
def format_vip_table_rich(bot=None) -> str:
    ...
    Kolom hanya No | Nama | Harga -- kolom Status (yang sebelumnya memanggai
    get_chat_member ke target_chat_id) sudah DIHAPUS. Lihat catatan lengkap
    di docstring format_vip_table() di atas soal alasannya.
