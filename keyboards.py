...
def vip_list_keyboard():
    ...
    (tidak dipaksa berpasangan dengan tombol "Kembali"). Tombol "⬅️ Kembali"
    sengaja tetap 1 baris sendiri di paling bawah, terpisah dari grid paket,
    supaya tetap jelas beda fungsi & gampang ditemukan."""
    packages = db.list_packages()
    package_buttons = [
        InlineKeyboardButton(
            f"{pkg['name']} - Rp{pkg['price']:,}".replace(",", "."),
