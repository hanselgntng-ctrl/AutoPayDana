...
- vip_packages(id, name, price, duration_days, description, link, target_chat_id, active)
...
def get_stats() -> dict:
    ...
    total_packages = _safe_scalar(cur, "SELECT COUNT(*) FROM vip_packages WHERE active=1")
    ...
    # Diagnostik -- kalau angka transaksi di atas terasa kurang (mis. "transaksi
    # lama tidak muncul"), bandingkan db_path di sini dengan config.DB_PATH yang
    # sedang dipakai bot.py: kalau BEDA, berarti ada lebih dari satu file bot.db
    # (mis. sisa file lama dari sebelum Railway Volume dipasang) dan stats ini
    # sedang membaca file yang salah. oldest_tx_created_at/newest_tx_created_at
    # membantu memastikan transaksi lama itu memang ADA di file yang sedang
    # dibaca atau tidak.
    db_path = DB_PATH
    oldest_tx_created_at = _safe_scalar(cur, "SELECT MIN(created_at) FROM transactions", default=None)
    newest_tx_created_at = _safe_scalar(cur, "SELECT MAX(created_at) FROM transactions", default=None)
    ...
    return {
        ...
        "db_path": db_path,
        "oldest_tx_created_at": oldest_tx_created_at,
        "newest_tx_created_at": newest_tx_created_at,
    }
