...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        greeting, parse_mode=ParseMode.MARKDOWN, reply_markup=kb.main_menu_keyboard()
    )
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚙️ *Menu Pengaturan Bot*", parse_mode=ParseMode.MARKDOWN, reply_markup=kb.settings_menu_keyboard())
    return ConversationHandler.END
...
async def settings_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "settings_back":
        await query.edit_message_text("⚙️ *Menu Pengaturan Bot*", parse_mode=ParseMode.MARKDOWN, reply_markup=kb.settings_menu_keyboard())
        return ConversationHandler.END

    if data == "set_greeting":
        await query.edit_message_text("Kirim teks sapaan baru (mendukung format Markdown):")
        return SET_GREETING

    if data == "set_vip_text":
        await query.edit_message_text("Kirim teks intro menu VIP baru:")
        return SET_VIP_TEXT

    if data == "set_qris":
        await query.edit_message_text("Kirim foto QRIS baru:")
        return SET_QRIS

    if data == "add_package":
        context.user_data["new_pkg"] = {}
        await query.edit_message_text("Masukkan *nama paket* baru (contoh: VIP Bulanan):", parse_mode=ParseMode.MARKDOWN)
        return ADD_PKG_NAME

    if data == "edit_package":
        await query.edit_message_text("Pilih paket yang ingin diedit:", reply_markup=kb.package_pick_keyboard("editpkg"))
        return EDIT_PKG_PICK

    if data == "delete_package":
        await query.edit_message_text("Pilih paket yang ingin dihapus:", reply_markup=kb.package_pick_keyboard("delpkg"))
        return ConversationHandler.END

    if data.startswith("delpkg_"):
        pkg_id = int(data.split("_")[1])
        db.delete_package(pkg_id)
        await query.edit_message_text("✅ Paket berhasil dihapus (dinonaktifkan).", reply_markup=kb.settings_menu_keyboard())
        return ConversationHandler.END
    ...
    if data.startswith("editpkg_"):
        ...
        await query.edit_message_text(
            ...
            "Masukkan *nama baru*:", parse_mode=ParseMode.MARKDOWN
        )
...
async def save_greeting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db.set_setting("greeting_text", update.message.text)
    await update.message.reply_text("✅ Teks sapaan berhasil diperbarui.", reply_markup=kb.settings_menu_keyboard())
    return ConversationHandler.END


async def save_vip_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db.set_setting("vip_menu_text", update.message.text)
    await update.message.reply_text("✅ Teks menu VIP berhasil diperbarui.", reply_markup=kb.settings_menu_keyboard())
    return ConversationHandler.END


async def save_qris(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.photo:
        await update.message.reply_text("Mohon kirim dalam bentuk foto.")
        return SET_QRIS
    photo = update.message.photo[-1]
    file = await photo.get_file()
    await file.download_to_drive(config.QRIS_IMAGE_PATH)
    await update.message.reply_text("✅ QRIS berhasil diperbarui.", reply_markup=kb.settings_menu_keyboard())
    return ConversationHandler.END


async def add_pkg_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["new_pkg"]["name"] = update.message.text
    await update.message.reply_text("Masukkan *harga* (angka saja, contoh: 50000):", parse_mode=ParseMode.MARKDOWN)
...
async def add_pkg_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Masukkan *durasi VIP* dalam hari (contoh: 30):", parse_mode=ParseMode.MARKDOWN)
    return ADD_PKG_DURATION


async def add_pkg_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.text.isdigit():
        await update.message.reply_text("Durasi harus berupa angka. Coba lagi:")
        return ADD_PKG_DURATION
    context.user_data["new_pkg"]["duration_days"] = int(update.message.text)
    await update.message.reply_text("Masukkan *deskripsi singkat* paket (atau kirim '-' untuk kosongkan):", parse_mode=ParseMode.MARKDOWN)
    return ADD_PKG_DESC
...
async def add_pkg_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Paket VIP baru berhasil ditambahkan.", reply_markup=kb.settings_menu_keyboard())
...
async def edit_pkg_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Masukkan *harga baru*:", parse_mode=ParseMode.MARKDOWN)
...
async def edit_pkg_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Masukkan *durasi baru* (hari):", parse_mode=ParseMode.MARKDOWN)
...
async def edit_pkg_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Paket berhasil diperbarui.", reply_markup=kb.settings_menu_keyboard())
...
def main():
    ...
    settings_conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(settings_router, pattern="^(set_greeting|set_vip_text|set_qris|add_package|edit_package|delete_package|delpkg_\\d+|editpkg_\\d+|settings_back|settings_close)$"),
        ],
        states={
            SET_GREETING: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_greeting)],
            SET_VIP_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_vip_text)],
            SET_QRIS: [MessageHandler(filters.PHOTO, save_qris)],
            ADD_PKG_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_name)],
            ADD_PKG_PRICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_price)],
            ADD_PKG_DURATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_duration)],
            ADD_PKG_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_desc)],
            ADD_PKG_CHATID: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_chatid)],
            ADD_PKG_LINK: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_pkg_link)],
            EDIT_PKG_PICK: [CallbackQueryHandler(settings_router, pattern="^editpkg_\\d+$")],
            EDIT_PKG_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_pkg_name)],
            EDIT_PKG_PRICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_pkg_price)],
            EDIT_PKG_DURATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_pkg_duration)],
            EDIT_PKG_CHATID: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_pkg_chatid)],
            EDIT_PKG_LINK: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_pkg_link)],
        },
