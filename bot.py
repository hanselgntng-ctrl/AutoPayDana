...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "show_vip":
        ...
        try:
            table_html = kb.format_vip_table_rich(query.from_user.id)
        ...
        table = kb.format_vip_table(query.from_user.id)
        text = f"{vip_text}\n\n{table}"
        try:
            await fade_transition(
...
async def start_purchase_flow(chat_id: int, telegram_user, context: ContextTypes.DEFAULT_TYPE,
    ...
    tx_id = db.create_transaction(telegram_user.id, username, pkg_id, final_amount, unique_code)
