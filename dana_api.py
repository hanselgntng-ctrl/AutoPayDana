...
def _generate_signature(payload: str) -> str:
    ...
    try:
        ...
        # Kalau private key belum di-setup ATAU library `cryptography` belum
        # ter-install, kembalikan string kosong secara diam-diam -- jalur API
        # resmi DANA Bisnis ini memang OPSIONAL (lihat docstring modul di atas).
        # Bot tetap bisa jalan normal lewat jalur notifikasi MacroDroid/OCR.
        return ""
...
def get_recent_mutations(minutes: int = None) -> list[dict]:
    ...
    """
    # Kalau jalur API resmi DANA Bisnis belum di-setup sama sekali (belum ada
    # DANA_API_BASE_URL), SKIP total tanpa mencoba request & tanpa nge-log
    # error apapun -- ini jalur OPSIONAL (lihat docstring modul di atas), dan
    # mencoba+gagal setiap kali fungsi ini dipanggil cuma bikin log penuh
    # noise padahal memang belum dikonfigurasi dengan sengaja.
    if not getattr(config, "DANA_API_BASE_URL", None):
        return []
...
def generate_unique_code(base_amount: int, tx_id: int) -> tuple[int, str]:
    """
    ...
    PENTING: basis kode unik di sini adalah `tx_id` (primary key transaksi di
    database, dijamin unik & permanen oleh SQLite), BUKAN counter di memori
    proses Python. `tx_id` tidak akan pernah reset/tabrakan biarpun bot
    restart berkali-kali -- beda dengan pendekatan lama (counter RAM) yang
    reset ke 0 setiap restart dan bisa bikin dua transaksi beda dapat kode
    unik yang SAMA, sehingga sistem pencocokan pembayaran otomatis salah
    mencocokkan ke transaksi yang keliru.

    Kembalikan (nominal_final, kode_unik_3_digit).
    """
    unique_suffix = (tx_id % 899) + 100  # angka 100-998
