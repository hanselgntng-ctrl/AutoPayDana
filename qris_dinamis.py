...
def decode_qris_image(image_path: str) -> str:
    ...
    Dipanggil sekali setiap kali admin upload/ganti gambar QRIS (bukan
    setiap kali ada pembelian) -- hasilnya disimpan di database lewat
    db.set_setting('qris_static_string', ...) supaya tidak perlu decode
    ulang gambar setiap transaksi. Lihat save_qris() di bot.py.

    Melempar QRISDecodeError kalau file tidak bisa dibaca atau tidak ada
    QR code yang kebaca sama sekali di gambar tsb (mis. gambar rusak/blur)."""
    img = cv2.imread(image_path)
    if img is None:
        raise QRISDecodeError(f"Gagal membuka file gambar: {image_path}")
...
def generate_qris_image(qris_string: str, output_path: str, size: int = 700) -> None:
...
def generate_qr_png_bytes(qris_string: str, size: int = 600) -> bytes:
    ...
    if not success:
        raise QRISDecodeError("Gagal encode gambar QR ke format PNG")
    return buf.tobytes()


def build_dynamic_qris_png(static_image_path: str, amount: int) -> bytes:
    """Fungsi gabungan (opsional dipakai langsung dari bot.py): baca QRIS
    statis dari `static_image_path`, sisipkan `amount`, kembalikan PNG
    (bytes) siap kirim. Melempar QRISDecodeError/ValueError yang sama
    seperti decode_qris_image()/inject_amount() kalau ada langkah yang
    gagal -- pemanggil WAJIB siapkan fallback ke gambar QRIS statis asli
    kalau ini gagal."""
...
def _parse_tlv(s: str):
    """Parse string EMVCo jadi list (tag, value). Tidak rekursif --
...
def inject_amount(qris_string: str, amount: int) -> str:
    ...
    Melempar ValueError kalau tag wajib (00/53) tidak ditemukan --
