...
def import_packages(packages: list) -> int:
    ...
    duplikat). Return jumlah baris yang berhasil di-restore."""
    count = 0
    with get_conn() as conn:
...
def import_transactions(transactions: list) -> int:
    ...
    INSERT OR REPLACE berdasarkan `id` -- idempotent. Return jumlah baris
...
def import_vip_members(vip_members: list) -> int:
    ...
    INSERT OR REPLACE berdasarkan `user_id` -- idempotent. Return jumlah
    baris yang berhasil di-restore."""
    count = 0
    with get_conn() as conn:
