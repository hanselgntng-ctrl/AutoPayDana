...
def import_data(data: dict) -> dict:
    """Kebalikan dari build_export_snapshot()/get_all_* di atas: mengembalikan
    (restore) data hasil /exportdata ke database.

    SEBELUM INI TIDAK ADA fungsi importer sama sekali di database.py --
    hanya ada sisi export (get_all_transactions, get_all_vip_members,
    get_all_packages, get_setting/set_setting satu-per-satu). Akibatnya
    backup JSON dari /exportdata tidak bisa dipulihkan kembali ke bot
    (misal setelah pindah server / volume Railway hilang) tanpa restore
    manual ke SQLite.

    `data` mengikuti struktur snapshot export, semua key OPSIONAL:
        {
            "settings": {"key": "value", ...},
            "packages": [{"id":.., "name":.., "price":.., "duration_days":..,
                           "description":.., "link":.., "target_chat_id":..,
                           "active":..}, ...],
            "transactions": [{...semua kolom tabel transactions...}, ...],
            "vip_members": [{...semua kolom tabel vip_users...}, ...],
        }

    Baris di-restore pakai INSERT OR REPLACE berdasarkan primary key (id /
    user_id / key) supaya idempotent -- import ulang snapshot yang sama
    tidak menghasilkan duplikat, dan record yang sudah ada akan ditimpa
    oleh isi snapshot. Semua tabel di-restore dalam SATU transaksi (lewat
    get_conn()) supaya kalau ada error di tengah jalan, tidak ada
    perubahan setengah-setengah yang tersimpan.

    Return dict berisi jumlah baris yang berhasil di-restore per tabel,
    contoh: {"settings": 8, "packages": 3, "transactions": 42, "vip_members": 5}
    """
    data = data or {}
    ...
    with get_conn() as conn:
        ...
        # -- vip_users --
        for member in (data.get("vip_members") or []):
            conn.execute(
